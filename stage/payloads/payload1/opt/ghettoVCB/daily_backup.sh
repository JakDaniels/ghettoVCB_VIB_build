#!/bin/sh
# daily backup script, run as a cron task
...
for ds in ${datastores}; do
	...
	if [ -d "${fpath}" ] && [ -f "${fpath}/ghettoVCB.conf" ] && [ -f "${fpath}/daily_backup_list.txt" ]; then
		# found a config so try and use it
		/opt/ghettoVCB/ghettoVCB.sh ${d} -g "${fpath}/ghettoVCB.conf" -f "${fpath}/daily_backup_list.txt"
	fi
done
